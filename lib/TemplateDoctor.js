...
class TemplateDoctor {
    ...
    _checkPkg() {
        ...
        if (!trim(this.pkg.version)) {
            ...
            if (!insVersion.valid) {
                this._error(
                    "Template version should comply with Semver. See https://semver.org",
                );
            }
        ...
        if (_getDocmaDepVersionIn(this.pkg, "dependencies")) {
            this._warn(
                'Remove "docma" dependency from your package. It\'s only required in "peerDependencies".',
            );
        }
        if (_getDocmaDepVersionIn(this.pkg, "devDependencies")) {
            this._warn(
                'Remove "docma" dev-dependency from your package. It\'s only required in "peerDependencies".',
            );
        }
        ...
        if (!peerInfo) {
            ...
            this._error(
                `"docma" peer-dependency does not have a valid range. Suggested range: "${peerInfo.suggestedRange}"`,
            );
        } else if (!peerInfo.minVersionOK) {
            this._error(
                `Minimum "docma" peer-dependency version must be "${MIN_DOCMA_VERSION}". Suggested Range: ">=${MIN_DOCMA_VERSION}"`,
            );
        } else if (!peerInfo.supportsNextVersion) {
            this._warn(
                `"docma" peer-dependency version range does not cover next Docma version. Are you OK with this? Suggested range: "${peerInfo.suggestedRange}"`,
            );
        }
        ...
        if (!_checkKeywords(this.pkg)) {
            this._warn(
                'Include keywords "docma" and "template" in your package.json.',
            );
        }
    }
    ...
    _checkStructure() {
        ...
        if (!fs.pathExistsSync(this.template.templateDir)) {
            this._error(
                'Missing "/template" directory in the root of the package.',
            );
        }
        ...
        if (
            !fs.pathExistsSync(licensePath) &&
            !fs.pathExistsSync(licensePathMD)
        ) {
    ...
    _tryBuilder() {
        try {
            ...
            this._error(
                `Template builder fails with given template. Your module probably exports an improper object.\n    ${chalk.red(
                    "Error: " + e.message,
                )}`,
            );
        }
    ...
    diagnose() {
        ...
        if (!this.settings.quiet) {
            ...
            console.log(
                chalk.white(
                    `Diagnostics completed with ${chalk.underline(res)}!`,
                ),
            );
            let statsMsg = chalk.gray(
                `  ${chalk.underline(nErrors)} error(s), ${chalk.underline(
                    nWarnings,
                )} warning(s).`,
            );

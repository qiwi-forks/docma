...
const indexFiles = [];
["index", "default", "main", "home", "readme", "guide"].forEach((name) =>
    indexFiles.push(`${name}.htm`, `${name}.html`)
);
...
module.exports = (spaPath, options) => {
    ...
    const baseIsSet =
        typeof options.base === "string" && options.base.length > 0;
    let targetPath = pathIsSet ? path.resolve(spaPath) : process.cwd();
    let basePath = baseIsSet ? options.base : "/";

    function checkPath() {
        if (!fs.pathExistsSync(targetPath)) {
            console.error(
                chalk.red(`Cannot serve "${spaPath}". Path does not exist!`)
            );
            return false;
        }
        return true;
    ...
    if (conf && !pathIsSet) {
        console.info(
            chalk.gray(`» Found configuration file @ ${docmaConfigPath}`)
        );
        console.info(
            chalk.gray("  I will attempt to serve the configured destination.")
        );
        console.info(
            chalk.gray(
                '  Provide a path (e.g. "./") to force-serve target directory...\n'
            )
        );
